      ...
      <div className="w-full max-w-[420px] mx-auto p-10 sm:p-14 bg-black/50 backdrop-blur-3xl border border-gold-dim/30 shadow-[0_0_80px_rgba(212,175,55,0.08),inset_0_0_20px_rgba(212,175,55,0.02)] relative z-10 rounded-2xl group/modal">
        ...
        {success ? (
          <div className="text-center animate-in fade-in zoom-in duration-500">
            <div className="bg-green-500/10 border-l-2 border-l-green-500 border-t border-t-green-500/20 border-r border-r-green-500/20 border-b border-b-green-500/20 text-green-400 p-6 mb-8 text-sm flex flex-col items-center justify-center gap-3 shadow-[0_0_15px_rgba(34,197,94,0.1)]">
              <CheckCircle2 className="w-10 h-10 shrink-0 text-green-500 mb-2" />
              <p className="font-semibold text-lg text-cream">Check your email</p>
              <p className="text-cream-dim text-center mt-1">
                We&apos;ve sent a password reset link to your email address. Please click the link to choose a new password.
              </p>
            </div>
            <Link href="/admin/login" className="inline-flex items-center gap-2 text-sm text-gold hover:text-gold-bright transition-colors uppercase tracking-widest mt-4">
              <ArrowLeft size={16} /> Back to Login
            </Link>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="flex flex-col gap-6">
            <div className="text-cream-dim text-sm text-center mb-2">
              Enter your email address and we&apos;ll send you a link to reset your password.
            </div>
            <div className="flex flex-col gap-2">
              <label htmlFor="email" className="text-[11px] uppercase tracking-widest text-gold-dim ml-1">Email Address</label>
              <input 
                required 
                type="email" 
                id="email" 
                ...
                className="w-full bg-black/40 border border-gold-dim/30 focus:border-gold px-4 py-3.5 text-cream outline-none transition-all duration-300 placeholder:text-cream-dim/30 hover:border-gold-dim/70 focus:bg-black/60 focus:shadow-[0_0_20px_rgba(212,175,55,0.15)] rounded-2xl"
              />
            </div>
            ...
            <div className="mt-4 text-center">
              <Link href="/admin/login" className="text-xs uppercase tracking-widest text-cream-dim hover:text-gold transition-all hover:drop-shadow-[0_0_8px_rgba(212,175,55,0.6)]">
                Back to Login
              </Link>
            </div>
          </form>
